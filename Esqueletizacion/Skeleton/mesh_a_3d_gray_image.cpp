...
int main(int argc, char* argv[]) {
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " <input.inr> <output.off>" << std::endl;
        return EXIT_FAILURE;
    }

    const char* input_inr = argv[1];
    ...
    Gray_level_image image(input_inr, 2.9f);
    ...
    CGAL::Surface_mesh_default_criteria_3<Tr> criteria(30., 1.5, 1.5);
